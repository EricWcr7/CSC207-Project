    ...
    public void execute(ShoppingListInputData shoppingListInputData) {
        ...
        for (String recipeName : recipeNames) {
            if (recipeName == null || "null".equals(recipeName)) {
                continue; // Skip null or "null" recipe names
            } else {
                Recipe recipe = recipeDataAccessObject.getOneRecipe(recipeName);
                String stringIngredients = recipe.getIngredients(); // Get ingredients as a formatted string

                // Parse the string into a Map
                String[] lines = stringIngredients.split("\n"); // Split by newline to get each ingredient
                for (String line : lines) {
                    String[] parts = line.split(": "); // Split each line by ": " to separate ingredient and measurement
                    if (parts.length == 2) { // Ensure it has both ingredient and measurement
                        String ingredient = parts[0].trim(); // Trim to clean up any whitespace
                        String measurement = parts[1].trim();
                        ...
                        if (ingredientsMap.containsKey(ingredient)) {
                            ...
                            String existingMeasurement = ingredientsMap.get(ingredient);
                            String combinedMeasurement = combineMeasurements(existingMeasurement, measurement);
